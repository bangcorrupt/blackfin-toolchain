...
Vagrant.configure("2") do |config|
  ...
  #
   config.vm.provider "virtualbox" do |vb|
    ...
    #vb.customize ["modifyvm", :id, "--usbehci", "off"]
     ...
     vb.customize ['usbfilter', 'add', '0', '--target', :id, '--name', 'Analog Devices, Inc. (White Mountain DSP) Blackfin USB Device', '--vendorid', '0x064b', '--productid', '0x0586']
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    apt-get install -y vim git build-essential make pkg-config
    ...
    rm -rf /vagrant/build-bfin-tools
